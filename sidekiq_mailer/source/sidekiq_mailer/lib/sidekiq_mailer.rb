...
module Sidekiq
  module Mailer
    @@excluded_enviroments = nil

    def self.excluded_enviroments=(envs)
      @@excluded_enviroments = [*envs].map{|e| e && e.to_sym}
    end
    ...
    def self.excludes_current_environment?
      !ActionMailer::Base.perform_deliveries || (excluded_enviroments && excluded_enviroments.include?(current_env.to_sym))
    end

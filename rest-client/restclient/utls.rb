module RestClient
  ...
  module Utils
    ...
    def self.get_encoding_from_headers(headers)
      ...
      _content_type, params = cgi_parse_headers(type_header)
    ...
    def self._cgi_parseparam(s)
      ...
      while s[0] == ';'
        ...
        ends = s.index(';')
        while ends && ends > 0 \
          && (s[0...ends].count('"') - s[0...ends].scan('\"').count) % 2 != 0
          ends = s.index(';', ends + 1)
        end
        if ends.nil?
          ends = s.length
    ...
    def self.cgi_parse_headers(line)
    ...
    #   => 'foo[a]=1&foo[a]=2'
    #
    # 得到一个数组， 数组中的元素是 "#{k}=#{v}" 最后再用 & join 起来
    ...
    # 将hash或者嵌套hash 转为嵌套数组。 外层是一个大的数组，内层是小的数组，每个数组是两个元素，第一个是key，第二个是value
    def self.flatten_params(object, uri_escape=false, parent_key=nil)
      unless object.is_a?(Hash) || object.is_a?(ParamsArray) || (parent_key && object.is_a?(Array))

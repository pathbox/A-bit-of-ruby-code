...
do_request_fiber = Fiber.new do |args|
  puts "Setting up HTTP request #1"

  f = Fiber.current
  http = EventMachine::HttpRequest.new('http://www.hao123.com/').get :timeout => 10
...
EM.run do
  do_request_fiber.resume(args)   # 第一次resume。 这里其实是 Fiber.new.resume
end
